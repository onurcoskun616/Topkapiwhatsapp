...
// Mesajı "okundu" olarak işaretle ve "yazıyor..." göstergesini aç
// (Meta, bunu en fazla ~25 sn veya yanıt gönderilene kadar gösterir).
export async function sendTypingIndicator(waMessageId) {
  const res = await fetch(`${API}/${process.env.WHATSAPP_PHONE_NUMBER_ID}/messages`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${process.env.WHATSAPP_TOKEN}`,
  ...
  if (!res.ok) throw new Error(`Yazıyor göstergesi hatası: ${res.status} ${await res.text()}`);
  return res.json();
}
